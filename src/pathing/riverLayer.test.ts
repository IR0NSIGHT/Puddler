...
describe("", () => {
  ...
  test("can collect one layer around one point", () => {
    const river = [{ x: 10, y: 10, z: 42, parent: { x: 10, y: 10, z: 42 } }];
    ...
    distances.forEach((p) => expect(p).toBeLessThanOrEqual(2));
    layerOne.forEach((p) => {
  ...
  test("layer parents are preferred to have lower z - 01", () => {
    ...
    layerOne
      .filter(
        (p) => squaredDistance(p, river[0]) == squaredDistance(p, river[1]),
      )
      .forEach((p) => {
  ...
  test("every point is in exactly one layer. no layers share points.", () => {
    ...
    const layers = collectLayers(river, 2).map((layer, idx) => ({
      idx: idx,
      layer: layer,
      set: makeSetFrom(layer),
    }));
  ...
  test("inspect why parenting is ", () => {
    ...
    allPoints.forEach((p) => expect(p.parent).toBeDefined());

    //non endpoint origins will have 1 + iterations + iterations children
    river.slice(1, river.length - 1).forEach((origin) => {
      const originChildren = allPoints.filter((p) =>
        pointsEqual(p.parent, origin),
      );
