...
const escapePond = (
  pond: Puddle,
  startPoint: parentedPoint,
  puddleDebugSet: SeenSet,
): {
  canEscape: boolean;
  ...
  if (pond.escapePoint !== undefined) {
    ...
    if (failed) {
      log(
        "ERROR: couldnt find path to escape point: " +
          JSON.stringify(escapePoint.point),
      );
 ...
 */
const nextRiverSegment = (
  from: parentedPoint,
  pondParams: PondGenerationParams,
  puddleDebugSet: SeenSet,
): {
  ...
  if (getZ(from.point) < params.waterLevel)
    //base water level reached
    return {
  ...
  const pond = findPondOutflow(
    [from.point],
    pondParams.maxSurface,
    puddleDebugSet,
  );
...
const onlyPointsBeforeMerge = (riverPath: parentedPoint[], rivers: SeenSet) => {
  const lastPointToAddIdx = findIndex<parentedPoint>(
    riverPath,
    (p: parentedPoint): boolean => {
      return rivers.has(p.point) || (params.stopOnWater && isWater(p.point));
    },
  );
  const didMerge = lastPointToAddIdx != -1;
  const pointsBeforeMerge = didMerge
    ? riverPath.slice(0, lastPointToAddIdx)
    : riverPath;
 ...
 * @param pondParams
 */
export const pathRiverFrom = (
  pos: point,
  rivers: SeenSet,
  pondParams: PondGenerationParams,
): River => {
  const path: parentedPoint[] = [
    { point: pos, parent: undefined, distance: -1 },
  ];
  let safetyIt = 0;
  ...
  while (safetyIt < 1000) {
    ...
    const { riverSegment, stopped, generatedPond } = nextRiverSegment(
      path[path.length - 1],
      pondParams,
      puddleDebugSet,
    );
    const { merge, pointsBeforeMerge } = onlyPointsBeforeMerge(
      riverSegment,
      rivers,
    );
    ...
    if (generatedPond) ponds.push(generatedPond);
    if (stopped || merge) {
      break;
    }
  }
...
export const insertInSortedQueue = (
  sortedQueue: parentedPoint[],
  point: parentedPoint,
): void => {
  let i = 0;
...
export function findClosestDrop(
  ...
  while (queue.length != 0 && safetyIterator < 50000) {
    ...
    const trueLowerNeighbours = neighbours.filter(
      (n) => getZ(n, true) < Math.round(posZ),
    );
